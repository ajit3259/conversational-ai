"""
Stage 6, pass B: barge-in.

Same pipeline as pass A, except the microphone stays live while the
assistant is talking. Speech detected during playback is treated as an
interruption: stop the audio and start a new utterance immediately.

Known limitation, and an expected one. Without headphones or echo
cancellation the microphone picks up the assistant's own voice and reads it
as a barge-in. The interruption logic here is right; the false triggers are
the echo problem, which pass C takes on.

Run with:  uv run stage6_orchestration/assistant_bargein.py
"""
...
def audio_callback(indata: np.ndarray, _frames: int, _time, status) -> None:
    ...
    if state["assistant_speaking"]:
        ...
        # Order matters here. sd.stop() is what wakes the main thread out of
        # sd.wait(), so the flag has to be false before that call or the
        # waking thread reads a value we are about to change.
        state["assistant_speaking"] = False
        sd.stop()
        print("barge-in detected")

        # Start the new utterance with this block. Dropping it clips the
        # first syllable off the interruption.
...
def speak(text: str) -> None:
    ...
    for chunk in tts_voice.synthesize(text):
        ...
        # sd.wait() returns both when a chunk finishes normally and when the
        # callback thread calls sd.stop(), so the flag is the only way to
        # tell a completed sentence from an interrupted one.
        if not state["assistant_speaking"]:
            break
